...
local Groups = require("vpanime-girl.groups")
...
function M.extras()
  local Extra = require("vpanime-girl.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/vpanime-girl/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/vpanime-girl/config.lua", "\n(--@class vpanime-girl%.Config.-\n})")
